...
import { Braces, Upload, Download, ArrowLeft, CheckCircle } from "lucide-react";
...
export default function XmlToJson() {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      if (!selectedFile.name.endsWith(".xml") && selectedFile.type !== "application/xml" && selectedFile.type !== "text/xml") {
        toast({
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.message || "Failed to convert XML");
      }
      ...
      toast({
        ...
        description: "XML file converted to JSON",
      });
    } catch (error: any) {
      ...
      <div className="p-6 max-w-4xl mx-auto">
        <div className="mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold" data-testid="text-page-title">XML to JSON</h1>
              <p className="text-muted-foreground">Convert XML documents to JSON format</p>
            </div>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Upload your XML file</CardTitle>
            <CardDescription>
              Convert XML data to structured JSON format
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            {file && uploadState === "idle" && (
              <div className="space-y-6">
                <div className="flex items-center gap-4 p-4 bg-muted/50 rounded-lg">
                  <Braces className="w-10 h-10 text-muted-foreground" />
            ...
            {(uploadState === "uploading" || uploadState === "processing") && (
              <div className="space-y-4">
                <div className="flex items-center gap-4 p-4 bg-muted/50 rounded-lg">
                  <Braces className="w-10 h-10 text-muted-foreground animate-pulse" />
                  <div className="flex-1">
